...
from twitch_app.src.classification.paths import leiden_file, large_twitch_edges, large_twitch_features
...
def NumberOfEdges(features, edges):
    ...
    features_with_edges_count = pd.merge(features_with_edges_count, edges_count, how='left', left_on='Node',
                                         right_on='numeric_id_2')
    features_with_edges_count['num_edges_target'] = features_with_edges_count['num_edges_target'].fillna(0)

    # Summing up edges
    features_with_edges_count['num_edges'] = features_with_edges_count['num_edges_src'] + features_with_edges_count[
        'num_edges_target']
    features_with_edges_count = features_with_edges_count.drop(
        ['numeric_id_1', 'numeric_id_2', 'num_edges_src', 'num_edges_target'], axis=1)
...
def PopularityRecommender(new_user_id, new_user_community):
    ...
    community = features_num_edges.loc[features_num_edges['Community'] == comm_number].sort_values(by='num_edges',
                                                                                                   ascending=False)
