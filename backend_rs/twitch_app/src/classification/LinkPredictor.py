...
class LinkPredictor:
    ...
    def train_link_predictor(self):
        ...
        try:
            ...
            # Parallel processing for calculating similarities
            with ThreadPoolExecutor(max_workers=6) as executor:
                X_train['similarity'] = list(executor.map(
                    lambda row: self.predict_link(row['source'], row['target']),
                    X_train.itertuples(index=False)))
    ...
    def predict_link(self, source, target):
        try:
            if str(source) not in self.model.wv or str(target) not in self.model.wv:
                return 0  # or handle in a way that's appropriate for your application
            ...
            logging.error(f"Error in predicting link: {e}")
    ...
    def predict_probability(self, source, target):
        ...
        if self.model is None or self.lr_model is None:
            raise ValueError("Models need to be trained before predicting.")
